...
In **Authentication → URL configuration**, add these to **Redirect URLs** (`useAuth.tsx` sets `emailRedirectTo` to **`http://localhost:3000/search`** in development and **`{origin}/search`** in production builds):

- `http://localhost:3000/search` (dev)
- Your production origin with `/search`, e.g. `https://your-domain.example/search`

If the site **Site URL** is only the origin (e.g. `http://localhost:3000`), the app also forwards auth hash callbacks to **`/search`** after the session is established.
